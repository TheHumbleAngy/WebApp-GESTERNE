<?php
    ...
    if (sizeof($_POST) > 0) {
        if (isset($_POST['opt'])) {
            ...
            switch ($option) {
                case 'numero':
                    $sql = "SELECT * FROM regulieres WHERE num_fact LIKE '%" . $element . "%'";
                    break;
                case 'ref':
                    $sql = "SELECT * FROM regulieres WHERE ref_fact LIKE '%" . $element . "%'";
                    break;
                case 'four':
                    $sql = "SELECT * FROM regulieres INNER JOIN fournisseurs ON regulieres.code_four = fournisseurs.code_four WHERE fournisseurs.nom_four LIKE '%" . $element . "%'";
                    break;
                case 'date_eta':
                    $sql = "SELECT * FROM regulieres WHERE dateetablissement_fact LIKE '%" . $element . "%'";
                    break;
                case 'date_rcp':
                    $sql = "SELECT * FROM regulieres WHERE datereception_fact LIKE '%" . $element . "%'";
                    break;
                case 'rem':
                    $sql = "SELECT * FROM regulieres WHERE remarques_facture LIKE '%" . $element . "%'";
                    break;
            }
            ...
            <?php }
                    ...
                    <div style='width: 400px; margin-right: auto; margin-left: auto'>
                        <div class='alert alert-info alert-dismissible' role='alert' style='width: 60%; margin-right: auto; margin-left: auto'>
                            <a href='form_principale.php?page=form_actions&source=regulieres&action=rechercher' type='button' class='close'
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <table class="table table-hover table-bordered">
                        ...
                        <?php
                            $sql = "SELECT * FROM regulieres ORDER BY dateetablissement_fact ASC";
                            if ($resultat = $connexion->query($sql)) {
                                if ($resultat->num_rows > 0) {
                                    $ligne = $resultat->fetch_all(MYSQLI_ASSOC);
                                    foreach ($ligne as $list) {
